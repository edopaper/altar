-- Apply after 018, before deploying the updated share-altar and frontend.
-- La dedicatoria viaja dentro de `altar_drafts.content` mientras es privada y
-- se copia a `altars.tribute` recién al publicar, igual que la fotografía.
begin;
alter table public.altars add column tribute jsonb;
...
create function public.my_altars() returns table (
  ...
  created_at timestamptz, revision integer, published_revision integer, updated_at timestamptz,
  is_published boolean, tribute jsonb
  ...
  select a.slug, coalesce(d.content->>'name', a.name), coalesce(d.content->'objects', a.objects),
    ...
    case when d.slug is null then a.tribute else nullif(d.content->'tribute', 'null'::jsonb) end
...
-- Igual que la versión de 017, más la copia de la dedicatoria al publicar.
create or replace function public.commit_altar_draft(
...
begin
  ...
  if p_publish then
    update public.altars set name = p_content->>'name', objects = p_content->'objects',
      photo_url = p_photo_url, cloth_color = p_content->>'clothColor',
      tribute = nullif(p_content->'tribute', 'null'::jsonb), is_published = true where slug = p_slug;
  end if;
