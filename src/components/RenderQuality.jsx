...
export default function RenderQuality({ forceAlways = false }) {
  const { low, preset, downgrade, reducedMotion } = useQuality()
  const setFrameloop = useThree((state) => state.setFrameloop)
  const invalidate = useThree((state) => state.invalidate)
  useEffect(() => {
    setFrameloop(forceAlways || !reducedMotion ? 'always' : 'demand')
    invalidate()
  }, [forceAlways, reducedMotion, setFrameloop, invalidate])
